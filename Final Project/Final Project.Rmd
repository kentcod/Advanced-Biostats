---
title: "Predicting Plant Invasion"
author: "Kent Codding"
date: "2023-11-14"
...
```{r}
Mpois$deviance/Mpois$df.residual
```

Far below 1.5, which shows again that overdispersion is not present.
...
Unfortunately, after removing all insignificant variables using the drop1 method, all variables remain insignificant. The hypothesis, that a GLMM or GLM could predict overall plant invasion based on continuous geological features like elevation above floodplain, was not supported. I hypothesize that this is due to a small sample size as contemporary literature suggests that a strong and linear signal exists between variables like elevation above floodplain and overall plant invasion. Further, confounders like the Riparian Buffer (good for conservation but bad for my model) may have skewed the signal between distance/ elevation from floodplain and overall plant invasion. 


Now, I will attempt to examine another dimension of this research project (no pun intended).

#Part II: Individual Species Abundances

Note: for my project pitch, I was not intending to include the collected data for individual species from the same research site... but then the following Tuesday we discussed multivariate analysis, which I thought could be interesting to apply to this high dimensional data.
...
Here, I loop through each row of the individual column of the dataframe (in the form of the created list) and extract information on the individual species listed there. I did this for each invasive species encountered in the data.
...
plot(1:8, strs, type = 'l', lwd = 2, 
     ...
     main = "Invasive Species scree plot")
```
...
```{r}
nm <- metaMDS(nmdf, noshare = F) #since my features are species, many of the default arguments in the metaMDS function (Bray-Curtis, k = 2, autotransform) are appropriate
```
...
```{r}
adonis2(nmdf ~ df$RB, permutations = 999)
```
the test rejects the null hypothesis that invasive plant communities are the same between areas affected and unaffected by the Riparian Buffer.

##### plot NMDS with ordiellipse

```{r, fig.height=10, fig.width = 6}
#plot site scores
par(mar = c(5, 4, 4, 5), mfrow = c(2,1), cex = .75)

plot(nm$points, pch = 21, bg = clr, 
cex = 1.5, main = "NMDS with ellipse of standard deviation",
ylab = "NMDS2", xlab = "NMDS1",
ylim = c(-1,1), xlim = c(-1,1))
...
text(.8, 1, 
     lab = paste("stress =", round(nm$stress,2), sep = " "), 
     cex = 1)
ordiellipse(nm, groups = df$RB, 
            kind = "sd", #draw standard deviations of points 
            conf = 0.95,
            draw = "polygon",
            col = c("yellow", "purple"))
...
#plot again with se
plot(nm$points, pch = 21, bg = clr, 
cex = 1.5, main = "NMDS with ellipse of standard error",
ylab = "NMDS2", xlab = "NMDS1",
ylim = c(-1,1), xlim = c(-1,1))
...
ordiellipse(nm, groups = df$RB, 
            kind = "se", #draw standard error of points 
            conf = 0.95,
            draw = "polygon",
            col = c("yellow", "purple"))
```
...
#add text
spp5 <- names(nmdf)[colSums(nmdf) > 5] #get names of species w/ greater than 5 occurences
text(nm$species[spp5,], lab = spp5)
...
check how PCs 1 and 2 for the predictor variables for Question 1 explain the most prevalent individual invaders. 
```{r}
...
Microstegium Vimineum and Celastrus Orbiculatis.
```{r}
#make a vector of colors that correspond to the different species
...
For question 1, my hypothesis was not supported as a GLM or GLMM could not viably predict overall invasion better than a neural network multilayer perceptron classifier. I belive that this is because the dataset was not big enough for GLMM (could not converge). I originally hypothesized that GLMM may be able to predict the random effects of the Riparian Buffer more accurately than the neural network, so a successful GLMM could have yielded a better model.
GLM also failed to find any significant predictors even with an interaction term for riparian buffer and the evident trend of distance from floodplain (heavily used in the machine learning model). This could be due to the small sample size and the difficulty of dealing with confounders like the Riparian Buffer.


For question 2, my hypothesis was supported as there is a difference between invasive plant communities for areas affected by or unaffected by the Riparian Buffer at Highland. Given the empirical data (in the form of maps), it is evident that the riparian buffer reduces overall invasion as most areas by the Massey Creek Riparian Buffer are yellow instead of orange or red like most areas by Houchins creek or upstream of the Massey Creek Riparian Buffer.

## Caveats
upland species were also included, so some difference could be explained by habitat difference rather than riparian buffer presence. However, given that the riparian buffer negatively correlates with overall invasion, it makes sense biologically that species like CEOR (Celastrus Orbiculatis) and MIVI (Microstegium Vimineum), which are both known indicators for overall invasion in Virginia, were found less in Riparian Buffer-affected areas.

## Future questions

regarding the riparian buffer (RB), more questions like individual species differences between RB-affected and non-RB-affected habitats. Further, a study with a larger sample size may be able to analyze indicator species analysis for each habitat. Evidently, effective riparian buffers remove invaders, so one would expect more invasive species to identify with the non-RB-affected habitat. However, some invasive species compete with each other for niches unoccupied by natives. Thus, if one invader like ARHI (joint head grass) hypothetically competes with MIVI and MIVI is greatly affected by riparian buffers, could joint head grass be more prevalent in RB-affected habitats and thus be an indicator for RB-affected habitats? This would be interesting to conservation ecology as tests like these could deem riparian buffers as effective for some species but uneffective for others. With limited conservation resources, if a stakeholder is dealing with a species like ARHI that does well in native riparian buffers, they may want to spend their money on a different conservation solution!